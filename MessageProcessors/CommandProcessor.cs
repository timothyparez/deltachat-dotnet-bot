...
public class CommandProcessor : IMessageProcessor
{
    ...
    {
        const string IMAGE_FILE = "delta.jpg";
        ...
        //By copying this from memory to disk we don't have to manage a separate jpg file
        var imageData = Convert.FromBase64String(imageBase64);
        File.WriteAllBytes(IMAGE_FILE, imageData);

        var filePath = Path.Combine(Environment.CurrentDirectory, IMAGE_FILE);
        MarkupLineInterpolated($"Current file path: [DeepPink3_1]{filePath}[/]");
